...
```
# Introducton
In this tutorial we briefly demonstrate the use of Dr Jon Yearsley's parallelised R coding of FreScaLO (Frequency Scaling using Local Occupancy). The original paper of Hill (2012) is [here](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210X.2011.00146.x).

# Required data
The data in the example are dummy data derived from the "unicorns" dataset in the [sparta](https://github.com/biologicalRecordsCentre/sparta) package, filtered and arranged into two time bins. The weights used are those created for Great Britain at the 10 x 10 km grid scale using UKCEH Land Cover Map information, also packaged within sparta. These data have all been pre-prepared for use with frescalo; guidance on these steps can be found within the sparta [vignette](http://biologicalrecordscentre.github.io/sparta/articles/sparta_vignette.html). Custom weights could be created using the `sparta::createWeights()` function, or other approaches (e.g. see [here](https://github.com/sacrevert/frescaloNeighbourhoods) and [here](https://github.com/sacrevert/createIDFneighbourhood)). In this example the sites are 10 x 10 km squares of the British National Grid (ESPG:27700) in lettered format, the species are just represented by arbitrary names and the time bins are of arbitrary length.
...
We now establish a computing cluster (here just on a local machine, but this can obviously be done on an HPC as well) using the `doParallel` library. We then also set a number of parameters and options for the frescalo algorithm.
...
R_star = 0.27 # Proportion of species used as benchmarks within neighbourhoods
...
Phi = 0.74 # Target value of neighbourhood local-frequency weighted mean frequency using to standardise sampling intensity across neighbourhoods
chunkSize = 250 # Number of sites or other data segments to pass to each compute node
```

We now prepare various required objects for the first part of the frescalo algorithm (subrouting "fresca" in the [original fortran code](https://www.brc.ac.uk/biblio/frescalo-computer-program-analyse-your-biological-records]). Broadly speaking, this part of the analysis equalises sampling intensity across all neighbourhoods in the analysis, in order to make them comparable for the subsequent trend assessment. Note that an alternative R translation of the original fortran is also available [here](https://github.com/sacrevert/fRescalo); this loop-based implementation is much slower than the current parallelised version, and was done for educational purposes rather than actual use.
...
The following animation provides a theoretical example of how this aspect of frescalo is intended to work. This was generated using an underlying multivariate Poisson point process (representing a community or species pool within a neighbourhood), combined with thinning (i.e. less than "perfect" sampling intensity relative to retrieving the true frequencies of species in the neighbourhood) and an iterative approach to then acheiving a target value of the frequency-weighted mean frequency (i.e. phi). The code behind this simulation can be found [here](https://github.com/sacrevert/Frequency-rescaling-demo). The red dot shown for each curve is its phi value (its frequency-weighted mean frequency).
...
Finally, we perform the frescalo trend analysis. In the original fortran code, this was subroutine "tfcalc". The `trend.out` object was simply described in the original frescalo README as "listing of time factors for species", although the standard deviations of these are also included. We have amended this function to also return the site x time period recording effort information. This comprises two important things: (i) Information on s_it, the proportion of benchmarks recorded for a site within a time period, and (ii), w, an additional weight used by Hill to reduce the influence of site x time period combinations which have very few benchmarks and which are therefore considered to have received no systematic sampling effort in a time period. This addition was not described in the main paper of Hill (2012), but it present in the fortran code and the original Frescalo README file packaged with the fortran executable. To quote Hill's README:
...
Apart from a couple of small discrepancies (which are likely due to the small, sparse dataset used for this example), the outputs are extremely close! Returning to general visualisation of the results, we can plot the trend for a species, along with the time factor standard deviations, as follows.
...
We can also produce the data required to make maps of the predicted species occupancies for different time periods, as first suggested by [Bijlsma (2013)](https://www.blwg.nl/mossen/onderzoek/rapporten/BLWGRapport15.pdf). This works by setting s_it to 1, and then using a species' time factor per time period to adjust a species all-time local frequency estimate (`freq_1` from the `freq.out` object returned by the `frescalo()` function above, or f\`_ij from the original paper of Hill 2012). Ideally these estimates would also take uncertainty in `freq_1` and in the time factor estimates into account, although we do not pursue that development here.
