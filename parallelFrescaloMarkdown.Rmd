...
authors: Drs Oli Pescott and Jon Yearsley
...
Finally, we perform the frescalo trend analysis. In the original fortran code, this was subroutine "tfcalc". The `trend.out` object was simply described in the original frescalo README as "listing of time factors for species", although the standard deviations of these are also included.
...
```{r message=TRUE, eval = TRUE, warning=FALSE, include=TRUE}
trend.out$time <- ifelse(trend.out$time == 1984.5, 1, 2) # recode back to original time bins
sppToPlot <- trend.out[trend.out$species=="Species 1",]
plot(1,type='n',
     ylim = c(0,1.0),
