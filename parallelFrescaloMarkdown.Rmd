...
## Introduction
In this tutorial we  demonstrate the use of Dr Jon Yearsley's parallelised R coding of FreScaLO (Frequency Scaling using Local Occupancy; "frescalo" hereafter). The original paper of Hill (2012) is [here](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/j.2041-210X.2011.00146.x){target="_blank"}. Dr Yearsley's original code is [here](https://github.com/DrJonYearsley/frescaloR){target="_blank"}, and all the code and functions relating to this tutorial can be found [here](https://github.com/sacrevert/frescaloR){target="_blank"} (this is a forked version of Dr Yearsley's repo with additional code and functionality). The paper for which Jon created his version of frescalo is [here](https://peerj.com/articles/7035/){target="_blank"}. Note also that an alternative R translation of the original fortran is also available [here](https://github.com/sacrevert/fRescalo){target="_blank"} — this loop-based implementation is much slower than the current parallelised version, and was done for educational purposes rather than actual use.

## Required data
The data in this example are dummy data derived from the "unicorns" dataset in the [sparta](https://github.com/biologicalRecordsCentre/sparta){target="_blank"} package, filtered and arranged into two time periods. The neighbourhood weights used are those created for Great Britain at the 10 x 10 km grid scale using UKCEH Land Cover Map information, also packaged within sparta. These data have all already been prepared for use with frescalo; guidance on these steps can be found within the sparta [vignette](http://biologicalrecordscentre.github.io/sparta/articles/sparta_vignette.html){target="_blank"}. Custom weights could be created using the `sparta::createWeights()` function, or other approaches (e.g. see [here](https://github.com/sacrevert/frescaloNeighbourhoods){target="_blank"} and [here](https://github.com/sacrevert/createIDFneighbourhood){target="_blank"}; there is also code for a RShiny neighbourhood weights visualisation tool [here](https://github.com/sacrevert/visualiseFresNeighbours/tree/main){target="_blank"}). In our example here the sites are 10 x 10 km squares of the British National Grid (ESPG:27700) in lettered format, the species are just represented by arbitrary names and the time bins are of unspecified lengths (for frescalo there is no particular restriction that all time periods in the analysis should be the same length).
...
We now prepare various required objects for the first part of the frescalo algorithm (subroutine "fresca" in the [original fortran code](https://www.brc.ac.uk/biblio/frescalo-computer-program-analyse-your-biological-records]){target="_blank"}). Broadly speaking, this part of the analysis equalises the all-time (i.e. across all time bins in the analysis) sampling intensity for each local neighbourhood in order to make them comparable for the subsequent trend assessment.
...
We can then fit a model for the linear trend for example, ideally respecting the uncertainty around the point estimates. Bootstrapped line ensembles  (see [here](https://www.sciencedirect.com/science/article/pii/S1470160X22005891){target="_blank"} for a frescalo example) or Bayesian meta-regression are both potential options here, depending on the aim and audience.
...
We can also produce the data required to make maps of the predicted species occupancies for different time periods, as first suggested by [Bijlsma (2013)](https://www.blwg.nl/mossen/onderzoek/rapporten/BLWGRapport15.pdf){target="_blank"}. This works by setting $s_{it}$ to 1, and then using a species' time factor per time period to adjust a species all-time local frequency estimate (`freq_1` from the `freq.out` object returned by the `frescalo()` function above, or $f'_{ij}$ from the original paper of Hill 2012). Ideally these estimates would also take uncertainty in `freq_1` and in the time factor estimates into account, although we do not pursue that development here.
